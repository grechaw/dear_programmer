...
Often the cause of this is that we said **Yes** to something early on in the project (ex. we use MVCC, we use microservices, we are agile, we write tests first, we  ...insert here ...) and never actively checked that everyone is marching forward together. Entropy, bitrot, changing requirements, new team members, and time all conspire to obscure initial assumptions so that they become myth ... not real but worshipped and given lip service from time to time but not actively managed. 
